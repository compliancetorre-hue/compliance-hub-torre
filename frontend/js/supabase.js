  ...
  try {
    ...
    if(riscos.length > 0) {
      DB.riscos = riscos.map(r => ({
        ...
        prob:r.prob||10, impacto:r.impacto||10, controle:r.controle||'',
        eficaciaControle:r.eficacia_controle||'Nenhuma', obs:r.obs||''
      }));
    }
...
async function sbSaveRisco(r) {
  if(!USE_SUPABASE) return;
  const row = { id:r.id, descricao:r.desc, cat:r.cat, filial:r.filial||'Todas', setor:r.setor, unidade:r.unidade||'Geral', prob:r.prob, impacto:r.impacto, controle:r.controle||'', eficacia_controle:r.eficaciaControle||'Nenhuma', obs:r.obs||'' };
