...
function exportRelatorioExcel() {
  if(typeof XLSX === 'undefined') { alert('Biblioteca de exportação ainda carregando. Tente novamente em instantes.'); return; }
  const items = getRelFiltered();

  const denunciasSheet = items.map(d => ({
    'Protocolo': d.proto, 'Data': formatDate(d.data), 'Filial': d.filial, 'Setor': d.setor||'',
    'Categoria': d.cat, 'Perigo': d.perigo, 'Status': d.status, 'Anonimato': d.anon,
    'Responsável': d.resp||'',
    'Relato (resumo)': (d.relato||'').substring(0,200),
    'Ação Inicial': (d.acaoInicial||'').substring(0,200),
    'Conclusão': d.conclusao||''
  }));

  const filiaisAll = [...new Set(items.map(d=>d.filial))].sort();
  const filialSheet = filiaisAll.map(f => {
    const fItems = items.filter(d=>d.filial===f);
    const encerradas = fItems.filter(d=>['Encerrada','Arquivada'].includes(d.status)).length;
    return {
      'Filial': f, 'Total': fItems.length,
      'Abertas': fItems.filter(d=>d.status==='Aberta').length,
      'Em Análise': fItems.filter(d=>d.status==='Em Análise').length,
      'Encerradas': encerradas,
      'Leve': fItems.filter(d=>d.perigo==='Leve').length,
      'Médio': fItems.filter(d=>d.perigo==='Médio').length,
      'Grave': fItems.filter(d=>d.perigo==='Grave').length,
      'Gravíssima': fItems.filter(d=>d.perigo==='Gravíssima').length,
      '% Resolução': fItems.length ? Math.round(encerradas/fItems.length*100) : 0
    };
  }).sort((a,b) => b.Total-a.Total);
  ...
  const setorSheet = Object.values(setorMap).map(s => ({
    'Setor': s.setor||'—', 'Filial': s.filial, 'Total': s.items.length,
    'Abertas': s.items.filter(d=>d.status==='Aberta').length,
    'Graves/Gravíssimas': s.items.filter(d=>['Grave','Gravíssima'].includes(d.perigo)).length,
    'Encerradas': s.items.filter(d=>['Encerrada','Arquivada'].includes(d.status)).length
  })).sort((a,b) => b.Total-a.Total);

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(denunciasSheet), 'Denúncias');
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filialSheet), 'Ranking Filiais');
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(setorSheet), 'Ranking Setores');
  XLSX.writeFile(wb, `relatorio_denuncias_${new Date().toISOString().split('T')[0]}.xlsx`);
}
