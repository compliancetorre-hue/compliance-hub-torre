...
Deno.serve(async (req) => {
  ...
  // Mesma contagem usada no Canal de Denúncia original: dias corridos
  // desde o registro, parando de contar quando a denúncia é fechada
  // (Encerrada/Arquivada) — não é "dias restantes", é "% do prazo
  // consumido", igual ao slaBar() do app administrativo.
  const concluida = ['Encerrada', 'Arquivada'].includes(data.status);
  const criadoEm = new Date(data.criado_em);
  const diasDecorridos = Math.max(0, Math.floor((Date.now() - criadoEm.getTime()) / 86400000));
  const pctSla = Math.min(Math.round((diasDecorridos / PRAZO_DIAS) * 100), 100);
